...
cs_maindata <- cs_maindata %>%
  ...
  ungroup() %>%
  mutate(
    time_to_second_hearing = as.numeric(difftime(second_hearing_date, filing_date, units = "days")) / 365.25
  )

cs_maindata <- cs_maindata %>%
  filter(time_to_second_hearing >= 0 | is.na(time_to_second_hearing)
         )
...
cs_survplot <- ggsurvplot(
  ...
  title = "Time to second hearing from filing for cases filed from Jan 2021 to Dec 2023",  
  surv.scale = "percent",              
  conf.int = TRUE,                     
...
print("Time to Second Hearing Statistics:")
for(i in seq_along(cs_timepoints)) {
  ...
  print(paste(
    "Cases getting its second hearing within",
    time_label,
    "of filing:",
