...
cs_maindata <- cs_maindata %>%
  filter(time_to_first_hearing >= 0 | is.na(time_to_first_hearing), 
         time_to_second_hearing >= 0 | is.na(time_to_second_hearing))
...
cs_filtereddata <- cs_filtereddata %>%
  mutate(
    event = ifelse(!is.na(time_to_second_hearing) & time_to_second_hearing >= 0
...
cs_survplot <- ggsurvplot(
  ...
  title = "Time to second hearing from first hearing for cases filed from Jan 2021 to Dec 2023",  
  surv.scale = "percent",              
  conf.int = TRUE,                     
...
for(i in seq_along(cs_timepoints)) {
  ...
  print(paste(
    ...
    time_label,
    ", given the first hearing happened within 3 months :",
